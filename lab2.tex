...
Cada trama ve donada està definida per
$$
x_i[n] =
\begin{cases}
...
\end{cases},
$$
i cadascuna d'aquestes es correspon amb el segment de $x$ que va des de $x[iM]$ fins a $x[i(M + 1) - 1]$ i afegint zeros fins que aquesta trama tingui mida $N$. Si volem que aquestes trames recorrin tota la senyal $x$ cal que el darrer $i$ que agafem contingui el darrer element de $x$, és a dir, $i = P - 1$ ha de ser el mínim tal que $L_x - 1 \leq iM + n$, per a $0 \leq n \leq M - 1$. Com que $L_x - 1 \leq iM + n \leq iM + M - 1$, d'on $L_x \leq (i + 1)M = PM$. Per tant, necessitem
$$
P \geq L_x/M.
$$
